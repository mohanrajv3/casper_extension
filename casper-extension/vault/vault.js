class CasperVault {
  ...
  bindEvents() {
    ...
    document.getElementById('registerDecoysBtn')?.addEventListener('click', () => this.registerPendingDecoys());
    document.getElementById('checkBreachNowBtn')?.addEventListener('click', () => this.checkBreachNow());
    document.getElementById('simulateAttackBtn')?.addEventListener('click', () => this.simulateAttack());
    document.getElementById('compromisedModeBtn')?.addEventListener('click', () => this.activateCompromisedMode());
  ...
  async loadSecurity() {
    ...
    const breachCount = (data.breachAlerts || []).length;

    const breachLabel = document.getElementById('breachStatusLabel');
    if (breachLabel) {
      breachLabel.textContent = breachCount > 0 ? `${breachCount} breach alerts detected` : 'No breaches detected';
    }

    const trapLabel = document.getElementById('trapKeyCount');
    if (trapLabel) {
      trapLabel.textContent = breachCount > 0 ? `${breachCount} breach alerts` : 'Decoy monitoring active';
    }

    const riskScoreValue = document.getElementById('riskScoreValue');
    const riskSeverity = document.getElementById('riskSeverity');
    const riskFactors = document.getElementById('riskFactors');
    if (riskScoreValue) riskScoreValue.textContent = String(data.risk?.score ?? 0);
    if (riskSeverity) riskSeverity.textContent = String(data.risk?.severity || 'LOW');
    if (riskFactors) {
      const factors = Array.isArray(data.risk?.factors) ? data.risk.factors : [];
      riskFactors.textContent = factors.length ? factors.join(', ') : 'No active risk factors';
    }

    const logStatus = document.getElementById('logIntegrityStatus');
    const logMeta = document.getElementById('logIntegrityMeta');
    if (logStatus) {
      logStatus.textContent = data.logIntegrity?.valid ? 'Chain verified' : 'Chain integrity failed';
    }
    if (logMeta) {
      const checked = Number(data.logIntegrity?.checked || 0);
      logMeta.textContent = data.logIntegrity?.valid
        ? `${checked} events checked`
        : `Broken at event index ${data.logIntegrity?.brokenAtIndex ?? -1}`;
    }

    const compromisedBtn = document.getElementById('compromisedModeBtn');
    if (compromisedBtn && data.compromisedMode?.active) {
      compromisedBtn.disabled = true;
      compromisedBtn.textContent = 'Compromised Mode Active';
    } else if (compromisedBtn) {
      compromisedBtn.disabled = false;
      compromisedBtn.textContent = 'Activate Compromised Mode';
    }

    await this.refreshDeceptionStatus();
  }
  ...
  applySettingsToForm(settings) {
    ...
    setChecked('deceptionEnabled', settings.deception?.enabled !== false);
    setValue('decoyCount', String(Number(settings.deception?.decoyCount ?? 2)));

    setValue('honeyBaseUrl', settings.honeyServer?.baseUrl || '');
    setValue('honeyApiToken', settings.honeyServer?.apiToken || '');
    setValue('honeyPollSeconds', String(Number(settings.honeyServer?.pollSeconds ?? 60)));
    setChecked('honeyAutoPoll', settings.honeyServer?.autoPoll === true);
  }
  ...
  async saveSettings() {
    const settings = {
      ...
      deception: {
        enabled: Boolean(document.getElementById('deceptionEnabled')?.checked),
        decoyCount: Number(document.getElementById('decoyCount')?.value || 2),
      },
      honeyServer: {
        baseUrl: document.getElementById('honeyBaseUrl')?.value?.trim() || '',
        apiToken: document.getElementById('honeyApiToken')?.value?.trim() || '',
        pollSeconds: Number(document.getElementById('honeyPollSeconds')?.value || 60),
        autoPoll: Boolean(document.getElementById('honeyAutoPoll')?.checked),
      },
      mailService: {
  ...
  async registerPendingDecoys() {
    const saved = await this.saveSettings();
    if (!saved) return;

    const response = await this.sendMessage({ type: 'REGISTER_PENDING_DECOYS' });
    if (!response.success && !response.registered) {
      if (response.requiresUnlock) return this.handleLockedSession();
      this.toast(response.message || 'Decoy registration failed', 'error');
      return;
    }

    this.toast(response.message || 'Decoys registration completed', 'success');
    await this.refreshDeceptionStatus();
  }
  ...
  async checkBreachNow() {
    ...
    const response = await this.sendMessage({ type: 'CHECK_BREACH_STATUS', force: true });
    if (!response.success) {
      if (response.requiresUnlock) return this.handleLockedSession();
    ...
    this.toast(response.message || 'Breach check complete', 'success');
    await this.loadSecurity();
  }

  async simulateAttack() {
    const ok = window.confirm('Simulate attacker using one registered decoy credential now?');
    if (!ok) return;

    const saved = await this.saveSettings();
    if (!saved) return;

    const response = await this.sendMessage({ type: 'SIMULATE_ATTACK' });
    if (!response.success) {
      if (response.requiresUnlock) return this.handleLockedSession();
      this.toast(response.message || 'Attack simulation failed', 'error');
      return;
    }

    this.toast(response.message || 'Attack simulation complete', 'success');
    await this.loadSecurity();
  }

  async activateCompromisedMode() {
    const ok = window.confirm(
      'Activate compromised mode? This disables autofill and save prompts, and switches to high-alert response mode.'
    );
    if (!ok) return;

    const response = await this.sendMessage({
      type: 'ACTIVATE_COMPROMISED_MODE',
      reason: 'manual_user_action',
    });

    if (!response.success) {
      if (response.requiresUnlock) return this.handleLockedSession();
      this.toast(response.message || 'Failed to activate compromised mode', 'error');
      return;
    }

    this.toast(response.message || 'Compromised mode activated', 'success');
    await this.loadSettings();
    await this.loadSecurity();
  }

  async refreshDeceptionStatus() {
    const response = await this.sendMessage({ type: 'GET_DECEPTION_STATUS' });
    if (!response.success) return;

    this.deceptionStatus = response.data || null;
    const status = this.deceptionStatus || {};

    const trap = document.getElementById('trapKeyCount');
    if (trap) {
      trap.textContent = `${status.registeredDecoys || 0}/${status.totalDecoys || 0} decoys registered`;
    }

    const state = document.getElementById('decoyRegistrationState');
    if (state) {
      const pending = Number(status.pendingDecoys || 0);
      const breached = Number(status.breachedDecoys || 0);
      state.textContent = breached
        ? `${breached} decoy(s) were triggered`
        : pending
          ? `${pending} decoy(s) pending registration`
          : 'All decoys registered';
    }
  }
